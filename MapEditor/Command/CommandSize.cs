...
namespace MapEditor.Command
{
    [SubCommandInformation(
       Name = "Size",
       Aliases = new string[] { "Siz", "Sz" },
       Description = "Size the selected Object",
       Permission = "ME.Edit",
       Platforms = new Platform[] { Platform.RemoteAdmin, Platform.ServerConsole },
       MainCommandName = "MapEditor",
       Usage = "Size \"to\" or move \"of\" and the amount or new cordonate",
       Arguments = new string[] { "to/of", "Vector" }
       )]
    internal class CommandSize : ISubCommand
    {
        public CommandResult Execute(CommandContext context)
        {
            ...
            {
                cusor.Scale(vector, false);

                result.Message = "Object sized";
                result.State = CommandResultState.Ok;
            }
            else if (context.Arguments.FirstElement().ToLower() == "of")
            {
                cusor.Scale(vector, true);
